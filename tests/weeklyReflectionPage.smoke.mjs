...
const actionPanel = await readFile(new URL('../src/renderer/src/components/reflection/WeeklyReflectionActionPanel.vue', import.meta.url), 'utf8');
...
assert.match(page, /重复问题/, 'reflection problems should mark repeated issues');
assert.match(page, /WeeklyReflectionActionPanel/, 'reflection page should delegate action workflow rendering');
assert.match(actionPanel, /待处理/, 'action panel should expose pending status');
assert.match(actionPanel, /已完成/, 'action panel should expose completed status');
assert.match(actionPanel, /未完成/, 'action panel should expose not-completed status');
assert.match(actionPanel, /上周改进动作复盘/, 'action panel should render previous action reviews');
assert.match(workflow, /generateWeeklyReflection/, 'workflow should call the typed generation API');
assert.match(workflow, /updateWeeklyReflectionImprovementStatus/, 'workflow should persist action status changes');
...
assert.match(preload, /weekly-reflection:update-improvement-status/, 'preload should expose the action status channel');
