...
import { Bot, ClipboardList, FileText, History, RefreshCw, Settings, Sparkles } from 'lucide-vue-next';
...
const { repos, selectedRepoPaths, loading, refreshRepos, toggleRepo, chooseWorkspace } = assistant;

const navItems = [
  { key: 'config', label: '日报配置', icon: Settings, enabled: true },
  { key: 'generate', label: '日报生成', icon: FileText, enabled: false },
  { key: 'sync', label: '同步任务', icon: ClipboardList, enabled: false },
  { key: 'history', label: '历史日志', icon: History, enabled: false },
  { key: 'system', label: '系统设置', icon: Settings, enabled: false },
];
  ...
  <aside class="sidebar">
    ...
    <div class="sidebar-projects">
      <div class="sidebar-projects-head">
        <span>项目列表</span>
        <el-button link type="primary" size="small" :icon="RefreshCw" :loading="loading" @click="refreshRepos">
          重新扫描
        </el-button>
      </div>
