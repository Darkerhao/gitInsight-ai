...
export async function callAiReport(
  ...
  const styleRequirements: Record<ReportPromptStyle, { name: string; instruction: string }> = {
    concise: {
      name: '简短精炼',
      instruction: '完整覆盖输入中已明确的工作事项，仅精炼表达，不得因篇幅遗漏。正文每条15-40字，按实际工作量自然分条。',
    },
    standard: {
      name: '标准均衡',
      instruction: '兼顾信息完整和阅读效率，完整覆盖输入中已明确的工作事项。正文每条25-70字，按实际工作量自然分条。',
    },
    detailed: {
      name: '具体详细',
      instruction: '在输入信息范围内完整说明具体改动对象、执行动作和实际结果，不得遗漏已明确的工作事项。正文每条40-100字，按实际工作量自然分条；工作量较少时不要凑数。',
    },
  };
...
2. 使用正式工作日报语言。
3. 按实际功能归纳，相同模块合并总结。
4. 各部分按实际工作量自然分条，不设条数上限；相同模块可合理合并，但不得遗漏已明确提供的事项。
