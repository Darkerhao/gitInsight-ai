...
export async function callAiReport(
  ...
  const styleRequirements: Record<ReportPromptStyle, { name: string; instruction: string }> = {
    concise: {
      name: '简短精炼',
      instruction: '只保留最关键的信息。正文每条15-40字，按实际工作量输出1-2条工作内容、1条工作成果和1条明日计划。',
    },
    standard: {
      name: '标准均衡',
      instruction: '兼顾信息完整和阅读效率。正文每条25-70字，按实际工作量输出1-3条工作内容、1-2条工作成果和1-2条明日计划。',
    },
    detailed: {
      name: '具体详细',
      instruction: '在输入信息范围内说明具体改动对象、执行动作和实际结果。正文每条40-100字，按实际工作量输出2-5条工作内容、1-3条工作成果和1-2条明日计划；工作量较少时不要凑数。',
    },
  };
...
2. 使用正式工作日报语言。
3. 按实际功能归纳，相同模块合并总结。
4. 用户补充多项工作时应合理合并，但不得遗漏已明确提供的事项。
5. 严格遵循本次风格要求控制篇幅和信息颗粒度。
6. 只能依据提供的Git数据、修改文件、代码变更和“用户补充的非 Git 工作内容”总结，禁止补写未出现的工作内容；用户补充内容必须被视为已确认事实。
7. “今日工作内容”每一条必须以轻量模块标签开头，格式为“【一级模块 / 具体功能】”。只有区分同名功能或定位必须依赖场景时，才使用“【一级模块 / 页面或场景 / 具体功能】”。
8. 模块标签优先从业务模块、页面、组件、接口、路由、配置项、字段语义或文件路径中归纳；无法确定完整层级时至少输出最具体的页面或功能名称，不要写“相关模块”。
9. 不要省略模块标签，不要输出“模块/功能：”这类说明文字，不要只输出问题描述或优化描述。
10. “工作成果”描述业务结果或质量收益，避免重复今日工作中的字段名、实现细节和排序逻辑。
11. “明日计划”聚焦回归验证、同类问题排查或风险收敛，避免复述今日工作。
12. 示例格式：1. 【资产数据大屏 / 合同到期预警列表】修复签订公司映射与剩余天数排序问题，确保预警列表展示准确。
13. 输出格式中的占位词仅用于说明格式，实际输出必须替换为真实模块、真实功能和真实工作内容。
