...
## 2026-08-23 一周日报第二、三阶段

### 目标

在“一周日报”页面完成内容可信度和操作效率增强，不改后端接口、数据库协议或单日报语义。

### 阶段

| 阶段 | 状态 | 验收 |
| --- | --- | --- |
| 1. 现状与设计 | complete | 明确每项能力复用的现有状态和最小实现 |
| 2. 内容可信度 | complete | 质量检查、证据抽屉、局部补写、重复提醒、模板可用 |
| 3. 操作效率 | complete | 沿用配置、最近项目、快捷键、失败重试、一键生成提交可用 |
| 4. 质量验证 | complete | 周报专项 19/19、全量测试、typecheck、build、diff check 通过 |

### 约束

- 所有改动限定在“一周日报”页面及其周报 renderer composable/shared helper。
- 不新增后端接口、不改变飞书 payload、不影响单日报页。
- 局部补写采用当前编辑器内容的可追踪追加，不伪造 AI 已生成结果。

## 2026-08-22 会议汇报周报模块

### 目标

按 `.ai_state/design/2026-08-22-weekly-summary.md` 实现独立周报模块：快速选择工作周，从有效日报生成可编辑、可保存的会议汇报周报，并支持历史、来源、复制和 Markdown 导出。

### 阶段

| 阶段 | 状态 | 验收 |
| --- | --- | --- |
| 1. 领域模型与存储测试 | complete | 日期范围、日报去重/有效性、AI 结构校验、Markdown 投影、SQLite upsert 测试通过 |
| 2. 主进程与 IPC | complete | 数据库迁移、AI Client、IPC、preload 和共享类型接通 |
| 3. 页面闭环 | complete | 周期快捷切换、范围选择、生成、编辑保存、历史、来源抽屉、复制导出完成 |
| 4. 质量验证 | complete | 周报专项 6/6、全量测试、typecheck、build、diff check 通过 |

### 约束

- 不改变单日报、一周日报、周反思和自动同步语义。
- 不自动发送飞书，不实现月报、季度总结或年度总结。
- 保留现有工作树中与本任务无关的用户改动。

## 2026-08-16 周反思改进动作闭环

### 目标

让每条改进动作可由用户标记待处理、已完成或未完成；下一期周反思自动读取紧邻上一期，AI 只依据本期日报给出状态建议和效果评估，并标记连续出现的问题。

### 阶段

| 阶段 | 状态 | 验收 |
| --- | --- | --- |
| 1. 分诊与设计 | complete | 状态与 AI 结果分离；只比较同项目、同来源范围的紧邻上一期 |
| 2. 领域与存储测试 | complete | 迁移、状态持久化、上一期选择、AI 引用校验和 Markdown 测试完成 |
| 3. 主进程与页面闭环 | complete | 单一状态更新 IPC、动作面板、AI 建议和重复问题展示完成 |
| 4. 质量与运行验证 | complete | 专项/全量/typecheck/build/diff 和 Electron 跨周闭环通过 |

### 约束

- AI 只建议状态，不自动覆盖用户状态。
- 日报未提及只能判定证据不足，不能推断未完成。
- 不新增动作任务表、提醒、向量检索或跨全部历史扫描。
- 保留现有周反思 v1 和无关 `.claude/worktrees/` 变更。
